...
router
  ...
  .post(
    validation(participantSchema.addParticipant),
...
router
  .route('/:userId')
  .all(
    validation(null, projectSchema.projectParam.concat(userSchema.userParam)),
  )
  .get(
    authorizeProjectPermission(['owner', 'member']),
    participantController.viewParticipant,
  )
